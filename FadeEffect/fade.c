...
void fade_mmx(SDL_Surface* im1,SDL_Surface* im2,Uint8 alpha, SDL_Surface* imOut)
{
	...
	Uint64 gA = (long long)(alpha) *2;
	Uint64 gB = (long long)(128 - alpha) *2;
	Uint64 goodAlpha = gA | (gA << 16) | (gA << 32) | (gA << 48);
	Uint64 goodBeta = gB | (gB << 16) | (gB << 32) | (gB << 48);
	
    	__asm__ volatile(	"\n movq mm0, %[gAl]"	//move quad
	    			"\n movq mm1, %[gBt]" 	
	    			"\n mloop:"
	    			"\n cmp %[out], %[end]"	// for out != end
	    			"\n je endPaint "
	    			...
	    			"\n pxor mm3, mm3"	//xor, clear
	    			"\n punpcklbw mm3, dword [ %[im2] ]"	//na zmiane unpakc
	    			"\n psrlw mm3, 8"	// w prawo bit	
	    			
	    			"\n pmullw mm2, mm0"	//wyklad mnozenie
	    			"\n pmullw mm3, mm1"
	    			"\n psrlw mm2, 8"	//nastepny czesc
	    			"\n psrlw mm3, 8"	//nastepny czesc
	    			"\n paddw mm2, mm3"	//dodaj
	    			"\n packuswb mm2, mm2"	//spakuj 
	    			"\n movd dword [ %[out] ], mm2"	//przygotuj out
	    			"\n add %[im1], 4"
	    			"\n add %[im2], 4"
	    			"\n add %[out], 4"
	    			"\n jmp mloop "
    	...
    	: [im1] "+r" (A), [im2] "+r" (B), [out] "+r" (out) : [end] "rm" (end), [gAl] "m" (goodAlpha), [gBt] "m" (goodBeta) : "mm0", "mm1", "mm3", "memory");
